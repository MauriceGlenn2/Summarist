...
export default function Landing() {
  const [greenTextOne, setGreenTextOne] = useState<number>(0);
  const [greenTextTwo, setGreenTextTwo] = useState<number>(0);
  ...
  useEffect(() => {
    const intervalIdSet1 = setInterval(() => {
        setGreenTextOne((prevIndex) => (prevIndex + 1) % headingsSet1.length);
    }, 3000);

    const intervalIdSet2 = setInterval(() => {
        setGreenTextTwo((prevIndex) => (prevIndex + 1) % headingsSet2.length);
    }, 3000);

return () => {  
    clearInterval(intervalIdSet1);
    clearInterval(intervalIdSet2);
}


  }, [headingsSet1.length, headingsSet2.length]);
